...
func Post(w http.ResponseWriter, r *http.Request) {
	...
	link := "https://medium.com/@dezhamartha17/apa-itu-whatsauth-api-dan-cara-menambahkan-location-pada-bot-whatapp-f41a8e9bfa15"
	if r.Header.Get("Secret") == os.Getenv("SECRET") {
		if msg.Message == "loc" || msg.Message == "Loc" || msg.Message == "lokasi" || msg.LiveLoc {
			...
			randm := []string{
				...
				"ada niatan apaan kak?",
				"mohon bersabar ya kaks",
				"silahkan bagikan lokasi kakak nya, biar di jemput",
				"kayanya mimin lagi afk nih",
			}
			dt := &wa.TextMessage{
				To:       msg.Phone_number,
				IsGroup:  false,
...
func Liveloc(w http.ResponseWriter, r *http.Request) {
	...
	reply := fmt.Sprintf("Hayoooo, kamu di %s \n kaaann, Koordinatenya : %s - %s\n", location,
